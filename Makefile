# This Makefile is for the scripts extension to perl.
#
...
TO_INST_PM = config.pl \
	full_sync.pl \
	sync.pl \
	sync2.pl

PM_TO_BLIB = full_sync.pl \
	$(INST_LIB)/full_sync.pl \
	sync.pl \
	$(INST_LIB)/sync.pl \
	config.pl \
	$(INST_LIB)/config.pl \
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  sync.pl $(INST_MAN3DIR)/sync.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               scripts' >> META_new.yml
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  full_sync.pl $(INST_LIB)/full_sync.pl \
	  sync.pl $(INST_LIB)/sync.pl \
	  config.pl $(INST_LIB)/config.pl \
	  sync2.pl $(INST_LIB)/sync2.pl 
